...
input_dir = 'in_silico_library_uff_min/'
checked_output = 'in_silico_library_uff_min_checked1/'
failed_labelling = 'in_silico_library_uff_min_failed1/'
pool_size = 64
...
def pool_handler(inp):
    ...
    try:
        mol = Chem.rdmolfiles.MolFromMol2File(input_dir + inp, sanitize = False)
    ...
    ## groups with sulfur , 33 = 2-thiophene, 34 = 2-furanyl, 55 = sulfide, 273 = ester, are S config by CIP rules
    ...
    # 5-position should have same CIP configuration if it is C2 symmetric
        ...
        return -1
    # make sure there's no weird indbox shit
    elif (pos4_label == 'aa') and ('R' in pos5):
        return -1
    # if it passes all of this it is chiral, C2 symmetric, R at 4,4'
    else: 
